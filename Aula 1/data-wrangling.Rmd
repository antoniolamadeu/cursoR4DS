...
# group_by + mutate (ou filtro) por posição
predicoes %>%
  mutate(rank_antes_do_group_by = rank(previsao)) %>%
  group_by(modelo) %>%
  mutate(rank_depois_do_group_by = rank(previsao)) 
  filter(rank(previsao) < 2) # os top 2 mais prováveis de ser A
  
# transformações deslocamento: lag ou lead
acoes_gather %>%
  group_by(acao) %>%
