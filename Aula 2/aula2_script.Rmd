...
re_conhe <- regex("conhec", ignore_case = TRUE)
re_nulo <- regex("anul|nul[ia]", ignore_case = TRUE)
re_dilig <- regex("dilig", ignore_case = TRUE)
re_deram <- regex("deram|provim|acolher", ignore_case = TRUE)

# todos os tipos de outros
outros <- c("Extinto", "Não conhecido", "Anulado", "Diligencia", "Outros")
...
decisoes <- dataset %>% 
  ...
  mutate(
    ...
    decisao = case_when(
      ...
      str_detect(decision, re_conhe) ~ "Nao conhecido",
      str_detect(decision, re_nulo) ~ "Anulado",
      str_detect(decision, re_dilig) ~ "Diligencia",
