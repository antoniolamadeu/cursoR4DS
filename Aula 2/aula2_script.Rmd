...
textos <- d_cjsg %>% 
  tidytext::unnest_tokens(word, ementa, token = "ngrams", n = 2) %>% 
  count(id, word) %>% 
  group_by(word) %>% 
  mutate(n = n()) %>% 
  ungroup() %>% 
...
d_final <- decisoes %>% 
  ...
  left_join(textos, "id") %>% 
  mutate_at(vars(starts_with("txt_")), funs(if_else(is.na(.), 0, .))) %>% 
...
# taxa de acerto
teste %>%
  mutate(res = predict(logistica, newdata = ., type = "response"),
...
TPR_aleatorio <- sapply(cortes, function(x) 
  tpr(df$decisao_bin, rep(1, length(df$predito)), x))

FPR_aleatorio <- sapply(cortes, function(x) 
  1 - tnr(df$decisao_bin, rep(1, length(df$predito)), x))
...
teste_com_mais_variaveis <- teste %>% 
  mutate(assunto = ifelse(str_detect(assunto, "Homicídio|Latrocínio"),
                          "Homicídio", assunto),
         assunto = ifelse(str_detect(assunto, "Roubo"), 
                          "Roubo", assunto),
         assunto = ifelse(str_detect(assunto, "Furto"), 
                          "Furto", assunto),
         assunto = ifelse(str_detect(assunto, "Estupro"), 
                          "Esturpo", assunto),
         assunto = ifelse(str_detect(assunto, "[tT]ráfico"), 
                          "Tráfico", assunto),
...
```{r}
arvore <- rpart::rpart(decisao_bin ~ autor + assunto + num_camara + 
                         n_movs + idade, data = teste_com_mais_variaveis)
rpart.plot::rpart.plot(arvore)
```
...
m <- glmnet::cv.glmnet(X, treino$decisao_bin, family = "binomial")

X_new <- model.matrix(decisao_bin ~ ., data = select(teste, -id))

teste %>% 
  mutate(res = predict(m, newx = X_new, 
                       s = "lambda.min", 
                       type = "response")[,1]) %>% 
...
```{r}
m <- randomForest::randomForest(decisao_bin ~ ., data = treino)
teste %>% 
  mutate(res = predict(m, newdata = teste, type = "class")) %>% 
  summarise(prop = sum(res == decisao_bin) / n()) %>% 
  pull(prop)
```
